...
export function isValidRating(mode: RatingMode, value: number): boolean {
  ...
  return Number.isInteger(value) && value >= config.minValue && value <= config.maxValue
}
 ...
 */
export function calculateAverageRating(mode: RatingMode, ratings: number[]): number | null {
