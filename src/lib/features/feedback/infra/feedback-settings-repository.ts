...
export class FeedbackSettingsRepository {
	...
	async update(data: UpdateFeedbackSettings): Promise<FeedbackSettings> {
		...
		if (data.sessionRatingMode !== undefined)
			updateData.sessionRatingMode = data.sessionRatingMode
		...
		if (data.feedbackIntroText !== undefined)
			updateData.feedbackIntroText = data.feedbackIntroText

		const record = await this.pb
			.collection('feedback_settings')
